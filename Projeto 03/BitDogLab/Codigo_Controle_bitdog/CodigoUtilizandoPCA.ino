...
// Definições dos canais do PCA9685
#define HeadServo_ch 0 // Servo da cabeça no canal 0
#define Leg1F_ch     1 // Perna 1 Frontal no canal 1
#define Leg1B_ch     2 // Perna 1 Traseira no canal 2
#define Leg2F_ch     3
#define Leg2B_ch     4
#define Leg3F_ch     5
#define Leg3B_ch     6
...
int pinoPositivo = 2;
int pinoTerraVirtual = 3;

// Função para converter ângulo em pulso PWM
int angleToPulse(float ang) {
  // Ajuste estes valores conforme necessário para seus servos
  // Valores típicos: 100 (0°) a 500 (180°) SG90
  return map(ang, 0, 180, 100, 500);
}
...
void setup() {
  ...
  pinMode(pinoPositivo, OUTPUT);
  digitalWrite(pinoPositivo, HIGH); 

  pinMode(pinoTerraVirtual, OUTPUT);
  digitalWrite(pinoTerraVirtual, LOW); 



  // Inicializa o PCA9685
  pwm.begin();
  pwm.setPWMFreq(60);  // Frequência de 60Hz para servos
...
void Servomovement() {
  ...
  // Atualiza todos os servos via PCA9685
  pwm.setPWM(Leg1F_ch, 0, angleToPulse(TOLeg1F));
  pwm.setPWM(Leg1B_ch, 0, angleToPulse(TOLeg1B));
  pwm.setPWM(Leg2F_ch, 0, angleToPulse(TOLeg2F));
  pwm.setPWM(Leg2B_ch, 0, angleToPulse(TOLeg2B));
  pwm.setPWM(Leg3F_ch, 0, angleToPulse(TOLeg3F));
  pwm.setPWM(Leg3B_ch, 0, angleToPulse(TOLeg3B));
  pwm.setPWM(Leg4F_ch, 0, angleToPulse(TOLeg4F));
  pwm.setPWM(Leg4B_ch, 0, angleToPulse(TOLeg4B));
  pwm.setPWM(HeadServo_ch, 0, angleToPulse(TOHeadservo));
}
...
void smoothmove() {
  ...
  if (maxstep > 0) {
    ...
    for (int i = 0; i <= maxstep; i += 1) {
      ...
      // Atualiza os servos via PCA9685
      pwm.setPWM(Leg1F_ch, 0, angleToPulse(LALeg1F));
      pwm.setPWM(Leg1B_ch, 0, angleToPulse(LALeg1B));
      pwm.setPWM(Leg2F_ch, 0, angleToPulse(LALeg2F));
      pwm.setPWM(Leg2B_ch, 0, angleToPulse(LALeg2B));
      pwm.setPWM(Leg3F_ch, 0, angleToPulse(LALeg3F));
      pwm.setPWM(Leg3B_ch, 0, angleToPulse(LALeg3B));
      pwm.setPWM(Leg4F_ch, 0, angleToPulse(LALeg4F));
      pwm.setPWM(Leg4B_ch, 0, angleToPulse(LALeg4B));
      
      delay(smoothdelay);
    }
  }

  // Movimento suave para o servo da cabeça
  if (LAHeadservo > TOHeadservo) {
    for (int i = LAHeadservo; i >= TOHeadservo; i -= 1) {
      LAHeadservo = i;
      pwm.setPWM(HeadServo_ch, 0, angleToPulse(LAHeadservo));
      delay(smoothdelay);
    }
  } else {
    for (int i = LAHeadservo; i <= TOHeadservo; i += 1) {
